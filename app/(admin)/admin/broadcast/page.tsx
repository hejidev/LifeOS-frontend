...
export default function AdminBroadcastPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setSentCount(null);
    sendBroadcast.mutate(form, { onSuccess: (d: any) => { setSentCount(d.sentTo); setForm({ title: "", message: "", audience: "ALL" }); } });
  }

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-8xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2"><Radio className="h-5 w-5 text-primary" /> Broadcast</h1>
        <p className="text-muted-foreground text-sm mt-1">Send a platform notification to a segment of users.</p>
      </div>
      <Card>
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full" disabled={sendBroadcast.isPending}><Send className="mr-2 h-4 w-4" /> {sendBroadcast.isPending ? "Sending..." : "Send broadcast"}</Button>
