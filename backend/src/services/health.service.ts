...
export async function getHealthSummary(userId: string) {
  ...
  const weekLogs = await prisma.healthLog.findMany({
  ...
  function computeTrend(value: number, target: number): "up" | "down" | "stable" {
...
export async function logHealth(userId: string, data: any) {
...
export async function createHabit(userId: string, data: any) {
