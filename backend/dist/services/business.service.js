...
// ── Dashboard ───────────────────────────────────────────────────────────
async function getDashboard(userId, range = "today") {
    ...
    const [sales, prevSales, customers, products, expenses] = await Promise.all([
        prisma_1.prisma.bizSale.findMany({ where: { userId, createdAt: { gte: start }, status: "PAID" }, include: { items: true, customer: true } }),
        prisma_1.prisma.bizSale.findMany({
            where: {
                userId,
                status: "PAID",
                createdAt: {
        ...
        prisma_1.prisma.bizExpense.findMany({ where: { userId, date: { gte: start } } }),
    ]);
    ...
    const metrics = [
        {
            ...
            change: revenueChange == null ? undefined : `${revenueChange >= 0 ? "+" : ""}${revenueChange}% vs prior period`,
        },
