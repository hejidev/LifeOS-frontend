...
function buildSystemPrompt(userName, context) {
    const lines = [
        ...
        ...(context.todayTasks.length
            ? context.todayTasks.map((t) => `- ${t.title} (${t.priority})${t.dueTime ? ` at ${t.dueTime}` : ""}`)
            : ["- none"]),
        "",
        `Overdue tasks (${context.overdueTasks.length}):`,
        ...(context.overdueTasks.length
            ? context.overdueTasks.map((t) => `- ${t.title} (${t.priority})${t.dueDate ? ` — due ${t.dueDate}` : ""}`)
            : ["- none"]),
        "",
        `Recent notes: ${context.recentNotes.map((n) => n.title).join(", ") || "none"}`,
        "",
        `Finance this month: spent $${context.finance.totalSpent} of $${context.finance.monthlyBudget} budget.${context.finance.insight
            ? ` ${context.finance.insight}`
            : ""}`,
        "",
        `Today's calendar events (${context.todayEvents.length}):`,
        ...(context.todayEvents.length
            ? context.todayEvents.map((event) => `- ${event.title} (${event.start} - ${event.end})`)
            : ["- none"]),
    ];
    return lines.join("\n");
}
async function streamAssistantReply(userId, userName, message, onDelta) {
    ...
    const context = await (0, ai_context_service_1.getAIContext)(userId);
    ...
    try {
        ...
        console.error("[ai-assistant] Gemini error:", error?.message ?? error);
        throw error;
    }
}
